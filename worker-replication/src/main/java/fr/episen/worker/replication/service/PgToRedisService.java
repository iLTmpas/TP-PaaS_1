...
public class PgToRedisService {
  ...
  public PgToRedisService(
      EmployeRepository employeRepository, RedisTemplate<String, String> redisTemplate) {
  ...
  public int replicateAllEmployes() {
    ...
    for (EmployeEntity employe : employes) {
      try {
        ...
        logger.error("Erreur réplication employé ID: {}", employe.getId(), e);
      }
    }
  ...
  public void replicateEmploye(EmployeEntity employe) {
    ...
    redisTemplate.opsForHash().put(key, "nom", employe.getNom());
    redisTemplate.opsForHash().put(key, "prenom", employe.getPrenom());
    redisTemplate.opsForHash().put(key, "mail", employe.getMail());
    redisTemplate.opsForHash().put(key, "valide", employe.getValide().toString());

    logger.debug(
        "Répliqué: badge {} → employé {} {} (valide: {})",
        employe.getBadgeId(),
        employe.getPrenom(),
        employe.getNom(),
        employe.getValide());
  ...
  /** Invalider le cache d'un badge */
  public void invalidateBadge(String badgeId) {
    String key = REDIS_KEY_PREFIX + badgeId;
    redisTemplate.delete(key);
    logger.info("Cache invalidé pour badge: {}", badgeId);
  }
}
