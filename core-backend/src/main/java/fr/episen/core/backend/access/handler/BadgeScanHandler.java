 ...
 *
 * <p>Flow : 1. Reçoit message MQTT 2. Lookup Redis 3. Décision autorisation 4. Publish commande
...
public class BadgeScanHandler {
  ...
  public void handleMessage(Message<?> message) {
    ...
    try {
      ...
      BadgeScanDTO scan =
          objectMapper.readValue(message.getPayload().toString(), BadgeScanDTO.class);
      ...
      if (isGranted) {
        // ✅ GRANTED
        log.info("✅ GRANTED - {} {} (ID: {})", prenom, nom, employeId);
        ...
        kafkaService.publishEntranceLog(employeId, scan.getBadgeId(), nom, prenom);

      } else {
        // ❌ DENIED
        log.warn("❌ DENIED - {} {} (ID: {}) - NOT_VALIDE", prenom, nom, employeId);
